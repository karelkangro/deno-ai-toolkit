...
if (!OPENAI_API_KEY || !CLAUDE_API_KEY) {
  console.error("❌ Please set OPENAI_API_KEY and CLAUDE_API_KEY environment variables");
  Deno.exit(1);
}
...
async function main() {
  ...
  try {
    ...
    const ragSystem = await createRAGSystem({
      ...
      systemPrompt: `You are a helpful assistant that can search through documents to answer questions.
Always search for relevant information before answering questions. Be specific and cite your sources.`,
    ...
    await ragSystem.addDocuments([
      {
        id: "deno-intro",
        content: "Deno is a modern runtime for JavaScript and TypeScript that is secure by default. It requires explicit permissions for file, network, and environment access.",
        metadata: { category: "technology", source: "documentation" },
      },
      {
        id: "deno-features",
        content: "Deno features built-in TypeScript support, ES modules, a built-in test runner, code formatter, and linter. It also has a standard library.",
        metadata: { category: "technology", source: "documentation" },
      },
      {
        id: "vector-db",
        content: "Vector databases enable semantic search by storing high-dimensional vectors. They allow finding similar documents based on meaning rather than exact keyword matches.",
        metadata: { category: "technology", source: "documentation" },
      },
      {
        id: "ai-agents",
        content: "AI agents are autonomous programs that can use tools and make decisions. They can be chained together to create complex workflows and solve multi-step problems.",
    ...
    const searchResults = await ragSystem.search("JavaScript runtime security", {
      limit: 3
    });
